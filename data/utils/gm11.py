...
class gm11(object):
    ...
    def __lsm(self):
        ...
        if self.bg_coff == []:
            ...
            for i in range(1, self.data_shape):
                background_values[i - 1][0] = (1 - self.bg_coff[i - 1]) * data_cum[i] + self.bg_coff[i - 1] * data_cum[
                    i - 1]
    ...
    def predict(self) -> np.array:
        ...
        for i in range(self.data_shape + 1, self.data_shape + 1 + self.predstep):
            self.pred_values[i - 1 - self.data_shape][0] = (1 - np.exp(a)) * (self.data[0][0] - b / a) * np.exp(
                -a * (i - 1))
